...
import schedule
import uuid
CameraEngine = importlib.import_module(f'camera-hik.camera_engine')
...
class IdentifyEngine:
    ...
    recognition_result = []   # 用于存放识别正确的人员结果
    # database = DB()
    ...
    def recognition(cls, image, face_encodings):
        ...
        if features is not None:
            ...
            if sim > (cls.database.get_identify_threshold() / 100.):
                job_num = cls.database.job_num_list[index]
                name = cls.database.name_list[index]
                ...
                for ipv4 in list(CameraEngine.frame_dict.keys()):
                    for time_stack in list(CameraEngine.frame_dict[ipv4].keys()):
                        ...
                        if success:
                            ...
                            cls.database.add_to_client(result_dict['工号'], result_dict['姓名'], device_group, file_path)
                        else:
                            device_group = CameraEngine.run_camera_info[ipv4]['设备组']
